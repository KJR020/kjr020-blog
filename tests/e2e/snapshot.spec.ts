...
// 実行時のcwdに依存させないため、このファイルからの相対位置で解決する。
const snapshotStylePath = fileURLToPath(new URL("./snapshot.css", import.meta.url));
...
test("スナップショット用CSSでAstro Dev Toolbarを非表示にする", async ({ page }) => {
  await page.setContent("<astro-dev-toolbar>Toolbar</astro-dev-toolbar>");
  await page.addStyleTag({ path: snapshotStylePath });
