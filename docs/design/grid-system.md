...
ページ全体を共通の2 / 6 / 12列へ分割する体系は採用しない。列構成はページパターンごとに定義する。
...
ホーム、記事一覧、検索の上限は、Tailwindの`container`の既定に従い、各Breakpointの値になる。記事詳細と同じ上限へ揃えるかは未決とし、決めるまでこの表を正本とする。
...
共通の12列Gridは採用しない。現在の要求である「本文と補助領域を並べる」「目次を閉じたら本文側へ幅を戻す」は、可変幅の主領域と固定幅の補助領域による2カラム構成で表現できる。12列へ移すと、固定幅の補助領域と収納時のRailを、12列体系の中の例外として説明することになる。共通化のための仕組みが既存の構成を説明しにくくするため、採用しない。
